...
const PostPage = () => {
  ...
  const onTextSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axiosClient.post(`/post/addPostItem/text`, {
        id,
        type: "text",
        content,
      });
  ...
  const isOwner = () => {
    console.log(post.author.id, user);
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button onClick={() => copyToClipboard(window.location.href)}>
            <img src={Share} alt="share" className="w-6" />
          </button>
          {isOwner() && (
            <button onClick={deletePost} className="bg-white">
          ...
          {isOwner() &&
            (postItemType == null ? (
              <div className="w-full h-64 bg-primary text-white rounded group cursor-pointer mt-4">
                <div className="flex flex-col justify-center items-center h-full group-hover:hidden ">
                  <img
                    src={Plus}
                    alt="Plus"
                    className="w-1/4 block fill-white"
                  />
                  <div>Add Item</div>
                </div>
              ...
              <form onSubmit={onTextSubmit}>
                <textarea
                  className="p-2 rounded text-black w-1/2 block"
                  id="content"
                  name="content"
                  rows={10}
                ></textarea>
              ...
              <form onSubmit={onImageSubmit} encType="multipart/form-data">
